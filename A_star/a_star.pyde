...
def new_grid():
    ...
    params.startCoordinates[0], params.startCoordinates[1] = randomSquare() # y/x
    params.endCoordinates[0], params.endCoordinates[1] = randomSquare()
    while params.endCoordinates[:] == params.startCoordinates[:]:
        params.endCoordinates[0], params.endCoordinates[1] = randomSquare()
    ...
    print("Start : ",params.startCoordinates)
    print("End : ",params.endCoordinates)
    astar = aStar(grid=params.grid, spawn=params.startCoordinates, goal=params.endCoordinates)

def randomSquare():
    global params
    x = int(random(0, len(params.grid[0])))
    y = int(random(0, len(params.grid)))
    return y,x
