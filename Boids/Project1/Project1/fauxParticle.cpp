	...
	void emmitter::update(float timeDelta, const glm::vec3& cameraPosition)
	{
		...
		for (int i = 0; i < numParticles; i++) {
			...
			if ((p.life > 0) && !(p.immortal)) {
				...
				//p.accelerate(explosionForce[i] - gravity, timeDelta);
				//p.accelerate(glm::vec3(0,0,0), timeDelta);
				p.cameraDistance = glm::length(p.position - cameraPosition);

				// Fill GPU buffer
				std::cout << p.life << "/" << maxLife/* << "," << p.position.z*/ << std::endl;
				...
				particlesContainer.colors[(4 * i) + 3] = 100.0f;
